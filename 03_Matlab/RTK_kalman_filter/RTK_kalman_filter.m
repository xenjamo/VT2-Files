...
% figure(1001); subplot(211) plot(t, rpyRPY(:,1:2) * 180/pi), grid on
...
% figure(1003) subplot(411) plot(t, mag_calib); grid on; subplot(412) plot(t,
...
% figure(1005); plot(t,sqrt(sum(accel.^2, 2))-sqrt(sum(accel_ENU.^2, 2)));
...
lat = hpllh(:,2)*pi/180;
h = hpllh(:,3);
% figure(1006) plot3(lon , lat, h), grid on title('llh'), xlabel('lon'),
% ylabel('lat'), zlabel('h')


pos_ecef = transformWGS84ToECEF_R(lat, lon, h); % R stands for rad

% figure(1007) plot3(pos_ecef(:,1) , pos_ecef(:,2) , pos_ecef(:,3)), grid on
...
% figure(1008) plot3(pos_nwu(:,1) , pos_nwu(:,2) , pos_nwu(:,3)), grid on,
...
% figure(1022) subplot(211) plot(t, wrapToPi(-headMot)) hold on;
